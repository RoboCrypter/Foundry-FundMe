...
import {EthToUsdConvertor} from "./EthToUsdConvertor.sol";
...
contract FundMe is ReentrancyGuard {
    ...
    using EthToUsdConvertor for uint256;
    ...
    function fund() external payable {
        if (msg.value.ethToUsdConversion(address(s_priceFeedAddress)) < MINIMUM_AMOUNT_IN_USD) {
            revert FundMe__Less_than_minimum_funding_amount();
        }
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        if (!success) {
            revert FundMe__Transfer_failed();
        }
    }
